...
from q2_sample_classifier.utilities import (
    ...
    _train_adaboost_base_estimator,
    _match_series_or_die,
    _extract_features,
)
from q2_sample_classifier import SampleEstimatorDirFmt, PickleFormat
...
class SampleEstimatorTestBase(SampleClassifierTestPluginBase):
    ...
    def _custom_setup(self, version):
        with open(
            os.path.join(self.temp_dir.name, "sklearn_version.json"), "w"
        ) as fh:
            fh.write(json.dumps({"sklearn-version": version}))
        shutil.copy(self.sklearn_pipeline, self.temp_dir.name)
        return SampleEstimatorDirFmt(self.temp_dir.name, mode="r")
...
class EstimatorsTests(SampleClassifierTestPluginBase):
    def setUp(self):
        ...
        self.exp_imp = pd.read_csv(
            self.get_data_path("importance.tsv"),
            sep="\t",
            header=0,
            index_col=0,
        ...
        self.exp_pred = pd.read_csv(
            self.get_data_path("predictions.tsv"),
            sep="\t",
            header=0,
            index_col=0,
            squeeze=True,
    ...
    def test_classify_samples_from_dist(self):
        ...
        expected = pd.Series(
            ("fat", "skinny", "fat", "skinny"), index=["f1", "s1", "f2", "s2"]
        )
        not_expected = pd.Series(
            ("fat", "fat", "fat", "skinny"), index=sample_ids
        )
    ...
    def test_classify_samples_from_dist_with_group_of_single_item(self):
        ...
        expected = pd.Series(
            ("skinny", "skinny", "skinny", "skinny"), index=sample_ids
        )
    ...
    def test_2nn(self):
        ...
        pred = res[0].view(pd.Series)
        expected = pd.Series(
            ("skinny", "fat", "fat", "skinny"), index=sample_ids
        )
        self.assertTrue(expected.sort_index().equals(pred.sort_index()))
    ...
    def test_classify_samples_w_all_train_set(self):
        with self.assertWarnsRegex(
            Warning, "not representative of " "your model's performance"
        ):
            table_fp = self.get_data_path("chardonnay.table.qza")
            table = qiime2.Artifact.load(table_fp)
    ...
    def test_classify_samples_ncv_accuracy(self):
        ...
        exp_pred = pd.Series(
            ...
            index=pd.Index(
                ["s4", "s6", "s1", "s10", "s5", "s8", "s2", "s9", "s3", "s7"],
                dtype="object",
                name="SampleID",
            ),
            name="prediction",
    ...
    def test_fit_regressor(self):
        ...
        exp_imp = pd.read_csv(
            self.get_data_path("importance_cv.tsv"),
            sep="\t",
            header=0,
            index_col=0,
        )
    ...
    def test_predict_classifications(self):
        for classifier in [
            ...
            ls_correct_range = [
                col
                for col in ls_pred_classes
                if prob[col].between(0, 1, inclusive=True).all()
            ]
            self.assertEqual(
                len(ls_correct_range),
                prob.shape[1],
    ...
    def test_predict_regressions(self):
        for regressor in [
            ...
            if regressor == "Ridge":
                self.assertAlmostEqual(
                    mse,
                    seeded_predict_results[regressor],
                    places=4,
                    msg="Accuracy of %s regressor was %f, but expected %f"
...
seeded_results = {
    ...
    "AdaBoostClassifier[ExtraTree]": 0.272727272727,
    "LinearSVC": 0.818182,
    ...
    "AdaBoostRegressor[ExtraTree]": 21.746031,
    "Lasso": 722.827623,
...
seeded_predict_results = {
    ...
    "AdaBoostClassifier[ExtraTree]": 21,
    "LinearSVC": 21,
    "SVC": 12,
    ...
    "AdaBoostRegressor[ExtraTree]": 33.95238095238095,
